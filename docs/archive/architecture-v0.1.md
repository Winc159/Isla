...
- 同一会话保存时使用进程锁、唯一临时文件和 `updatedAt` 冲突检查；并发进程不会静默覆盖彼此的消息，失效锁可在原进程已退出后回收。
- 单个损坏或不兼容的会话文件会被跳过并输出不含会话内容的警告，不阻断其他会话启动和选择。
...
- Provider 返回有效回答但 assistant 消息持久化失败时，回滚该 assistant 内存消息，避免下一轮模型看到用户未曾收到且磁盘无法重建的隐藏回答。
...
- CLI 命令在统一注册表中声明名称和说明；`/help` 从同一注册表生成命令列表，并展示当前输入快捷键。slash token 提示同时显示命令说明。
- 输入区和命令提示按终端列宽计算视觉行数；窄终端中的候选说明换行不会破坏光标恢复和区域清理。

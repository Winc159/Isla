...
参考 DSH 的 step/tool pipeline 不变量：每个模型 step 接收当前会话历史与可见工具 schema；模型产生具体 Tool Call 后，Tool Runtime 才执行权限判断和 Approval。

1. `ChatSession` 不再预先把 turn 分类成 answer、inspect、discuss 或 execute。
2. 每个启用 Tool 的 step 都提供当前完整 Tool schema，模型决定直接回答或调用 Tool。
3. Tool Call 产生后才进入 ToolRuntime、Permission、Sandbox 和 Approval。
4. Tool Result 加入当前 step 历史后继续调用模型，直到模型不再请求 Tool 或触发有界停止条件。
5. 删除执行前二次确认、强制 `tool_choice`、execution phase 及其专用 Prompt。
6. 删除不再被主链路消费的 IntentClassifier、ContextResolver 和 CompletionChecker；需要上下文压缩时以真实长度和效果数据重新设计。

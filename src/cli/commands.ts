...
const registeredCommands = [newCommand, sessionsCommand, memoryCommand, helpCommand, exitCommand];
...
export function findCliCommand(line: string): CliCommand | undefined {
  return commands.get(line) ?? (line.startsWith('/memory ') ? commands.get('/memory') : undefined);
}

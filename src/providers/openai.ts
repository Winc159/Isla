...
import type { ModelCallOptions, ModelProvider, ModelRequest, ModelResponse, ProviderCapabilities } from "../core/types.js";
...
class OpenAIProvider implements ModelProvider {
  readonly id = "openai"; readonly model: string; readonly streamingEnabled: boolean; readonly capabilities: ProviderCapabilities; private readonly client: OpenAI;
  constructor(config: OpenAIConfig) { this.model = config.model; this.streamingEnabled = config.streaming !== false; this.capabilities = { toolCalling: false, nativeStreaming: this.streamingEnabled, streamingToolCalls: false }; this.client = new OpenAI({ apiKey: config.apiKey, timeout: config.timeoutMs, maxRetries: 0 }); }

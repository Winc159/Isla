...
export function createProjectFilesCapability(projectRoot: string): ToolCapability {
  return {
    ...
    instructions: [
      ...
      "需要创建或修改文本文件时使用 write_text_file；该操作需要用户批准，工具也可以创建不存在的父目录。",
    ...
    tools: [createListDirectoryTool(projectRoot), createReadTextFileTool(projectRoot), createWriteTextFileTool(projectRoot)],
  };
}

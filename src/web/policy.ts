...
export function validateFetchUrl(input: string, allowedHosts: readonly string[], allowedUrls: readonly string[] = []): URL {
  ...
  if (!allowedHosts.includes("*") && !allowedHosts.includes(url.hostname.toLowerCase()) && !allowedUrls.includes(normalized)) throw new WebFetchError("WEB_HOST_NOT_ALLOWED", "目标 hostname 不在 allowlist 中。");
  return url;
}
...
export function resolveRedirect(location: string, base: URL, allowedHosts: readonly string[], allowedUrls: readonly string[] = []): URL {
  let target: URL;
  try { target = new URL(location, base); } catch (error) { throw new WebFetchError("WEB_REDIRECT_BLOCKED", "重定向目标无效。", error); }
  const validated = validateFetchUrl(target.toString(), allowedHosts, allowedUrls);

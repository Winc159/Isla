...
describe.skipIf(!enabled || !configured)("real v0.2.3 project search", () => {
  it("uses search_project and returns a verifiable source", async () => {
    ...
    try {
      ...
      child.stdin.write(`${JSON.stringify({ type: "prompt", id: "project-search-1", text: "请使用 search_project 工具搜索项目中关于 ORBIT-731 的事实，不要使用其他文件工具；然后回答它出现在哪个相对路径和第几行。回答中依赖该片段的句子后，必须原样附上工具返回的完整 [[source:project:v1:<64位小写十六进制>]] 标记。" })}\n`);
      ...
      expect(events.find(event => event.type === "response_end")?.projectSources).toEqual(expect.arrayContaining([{ path: "PROJECT-FACT.md", startLine: expect.any(Number) }]));

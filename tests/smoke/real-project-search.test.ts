...
describe.skipIf(!enabled || !configured)("real v0.2.3 project search", () => {
  it("uses search_project and returns a verifiable source", async () => {
    ...
    try {
      await waitFor(() => events.some(event => event.type === "ready"), 30_000);
      ...
      await waitFor(() => events.some(event => event.type === "response_end"), 90_000);
...
async function waitFor(predicate: () => boolean, timeoutMs: number): Promise<void> {
  ...
  while (!predicate()) { if (Date.now() - started > timeoutMs) throw new Error("Timed out waiting for real project search event"); await new Promise(resolvePromise => setTimeout(resolvePromise, 100)); }
}

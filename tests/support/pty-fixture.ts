...
export async function createPtyFixture(): Promise<{ readonly root: string; readonly workspace: string; readonly server: OpenAI_fixtureServer; readonly driver: PtyDriver; readonly dispose: () => Promise<void> }> {
  ...
  await writeFile(configPath, JSON.stringify({ version: 1, defaultProfile: 'pty', profiles: { pty: {

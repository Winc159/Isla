...
describe("tool capability composition", () => {
  it("always includes project files and omits disabled optional capabilities", () => {
    ...
    expect(capabilities.map(capability => capability.id)).toEqual(["project-files", "project-discovery", "command-execution", "task-state"]);
  ...
  it("adds enabled web tools through the same composition entry", () => {
    ...
    expect(capabilities.map(capability => capability.id)).toEqual(["project-files", "project-discovery", "command-execution", "task-state", "web"]);
    expect(capabilities[4]?.tools.map(tool => tool.definition.name)).toEqual(["web_search"]);
  });
  ...
  it("adds user interaction only when an input adapter is available", () => {
    ...
    expect(capabilities.map(capability => capability.id)).toEqual(["project-files", "project-discovery", "command-execution", "task-state", "user-interaction"]);
    expect(capabilities[4]?.tools.map(tool => tool.definition.name)).toEqual(["ask_user_question"]);
  });
});
